...
if show_chat:
    ...
    with st.container():
        ...
        if c1.button("❓ 적용공제세액이 계산된 근거를 알려주세요"):
            st.session_state["pending_user_text"] = "적용공제세액이 계산된 근거를 알려주세요"
        if c2.button("❓ 세액 1,000만원 이상 공제하려면 상시근로자를 몇 명 더 고용해야 할까요?"):
            st.session_state["pending_user_text"] = "세액 1000만원 이상 공제하려면 상시근로자를 몇 명 더 고용해야 할까요?"
        if c3.button("❓ 사후관리 결과를 바탕으로 공제세액이 추징세액보다 크려면 상시근로자를 몇 명 더 고용해야 할까요?"):
