...
# 환경변수 로드 (.env에 OPENAI_API_KEY 저장 가능)
load_dotenv()
...
# 사이드 설정 (모델, 온도, 시스템 프롬프트)
with st.expander("🔧 챗봇 설정", expanded=False):
    model = st.selectbox("모델 선택", ["gpt-4o-mini", "gpt-4o"], index=0, help="가벼운 gpt-4o-mini가 비용 효율적입니다.")
...
if user_text:
    ...
    with st.chat_message("assistant"):
        ...
        # stream tokens
        try:
            for token in stream_chat(st.session_state.chat_history, system_prompt=st.session_state.system_prompt, model=model):
                acc += token
