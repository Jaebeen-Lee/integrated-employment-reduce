...
if run:
    if params is None:
        ...
        # 🔒 결과를 세션에 보존하여 rerun(예: 챗 입력)에도 유지
...
# ──────────────────────────────────────────────────────────
# 💾 저장된 계산 결과가 있으면(예: 채팅으로 rerun 되었을 때도) 계속 보여주기
if "last_calc" in st.session_state and st.session_state.last_calc:
    lc = st.session_state.last_calc
    st.subheader("최근 계산 결과 (고정 표시)")
    mc1, mc2, mc3 = st.columns(3)
    mc1.metric("총 공제액", f"{lc['gross']:,} 원")
    mc2.metric("적용 공제액", f"{lc['applied']:,} 원")
    mc3.metric("추징세액 합계", f"{lc['total_clawback']:,} 원")
    st.caption(f"기업규모={lc['company_size']} · 지역={lc['region']} · 유지기간={lc['retention_years']}년 · 추징방식={lc['clawback_method']}")

    import pandas as _pd
    _schedule_df = _pd.DataFrame(lc["schedule_records"])
    st.dataframe(_schedule_df, use_container_width=True)
    c1, c2 = st.columns([1,1])
    with c1:
        if st.button("🧹 최근 결과 지우기", help="세션에 저장된 결과를 초기화합니다."):
            st.session_state.last_calc = None
            st.experimental_rerun()
    with c2:
        st.download_button(
            "⬇️ 최근 결과 스케줄 다운로드 (CSV)",
            data=_schedule_df.to_csv(index=False).encode("utf-8-sig"),
            file_name="credit_schedule_last.csv",
            mime="text/csv",
        )
