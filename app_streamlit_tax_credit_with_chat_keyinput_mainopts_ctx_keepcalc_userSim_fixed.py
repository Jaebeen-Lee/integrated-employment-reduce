...
if run:
    if params is None:
        ...
        # 시뮬레이션 편집용 DF를 세션에 저장하여 입력 중 rerun(예: 편집/채팅)에도 유지
        ...
        if st.button("🔁 추징세액 계산하기", type="primary", key="btn_compute_clawback"):
            ...
            # rerun에도 유지되도록 저장 (total_clawback 포함)
            ...
            # 최근 계산한 스케줄이 있으면 표시(버튼 누르기 전에는 표만 보이게)
    ...
    if st.session_state.get("last_calc"):
        ...
        if st.session_state.get("show_sim") and st.session_state.get("last_calc"):
            st.subheader("② 사후관리(추징) 시뮬레이션 - 다년표")
            import pandas as _pd
