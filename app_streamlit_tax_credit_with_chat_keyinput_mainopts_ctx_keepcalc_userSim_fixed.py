...
if run:
    ...
    if st.session_state.followup_table is None:
        years = list(range(1, retention_years + 1))
        init_rows = [{"연차": yr, "사후연도 상시": int(curr_total), "사후연도 청년등": int(curr_youth)} for yr in years]
        st.session_state.followup_table = pd.DataFrame(init_rows)
...
if st.session_state.last_calc is not None:
    ...
    # 사후표 결과가 이미 있다면 요약 아래 다시 보여줌 (공제액은 지우지 않음)
    st.subheader("③ 최근 추징 계산 결과")
    _schedule_df = pd.DataFrame(lc["schedule_records"])
    if not _schedule_df.empty:
        st.dataframe(_schedule_df, use_container_width=True)
        st.metric("추징세액 합계", f"{lc['total_clawback']:,} 원")
...
def _build_excel():
    ...
    if st.session_state.last_calc is not None:
        data.append(["추징세액 합계", int(st.session_state.last_calc.get("total_clawback", 0))])
    ...
    # 다년 추징표 시트 (last_calc가 있을 때만)
    if st.session_state.last_calc is not None:
        ws2 = wb.create_sheet("Clawback Schedule")
        headers = ["연차", "사후연도 상시", "사후연도 청년등", "추징세액"]
        ws2.append(headers)
        for row in st.session_state.last_calc["schedule_records"]:
            ws2.append([row["연차"], row["사후연도 상시"], row.get("사후연도 청년등", 0), row["추징세액"]])

        for cell in ws2[1]:
            cell.fill = header_fill; cell.border = border_all; cell.alignment = center; cell.font = Font(bold=True)

        for r in range(2, 2 + len(st.session_state.last_calc["schedule_records"])):
