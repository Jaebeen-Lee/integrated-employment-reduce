...
def _render_simulation_pane(params, size, region, clawback_method):
    ...
    - 표는 최초 한 번만 현재 요약값(curr_total/curr_youth)으로 초기화
    ...
    if reset or st.session_state.get("sim_df_init_key") != init_key:
        # 요약이 바뀐 경우에도 사용자가 원하면 재초기화 (자동 초기화 X -> 편집값 보호)
        st.session_state.sim_df = _pd.DataFrame(
            [{"연차": yr, "사후연도 상시": curr_total, "사후연도 청년등": curr_youth} for yr in years]
        )
        st.session_state.sim_df_init_key = init_key
...
def _render_summary_section():
    """세션의 summary가 있으면 ① 공제액 계산 결과를 항상 표시"""
    if not st.session_state.get("summary"):
        return
    _s = st.session_state.summary
    st.subheader("① 공제액 계산 결과")
    st.metric("총공제액 (최저한세/한도 전)", f"{int(_s['gross']):,} 원")
    st.metric("적용 공제액 (최저한세/한도 후)", f"{int(_s['applied']):,} 원")
    st.write(f"유지기간(사후관리 대상): **{int(_s['retention_years'])}년**")
